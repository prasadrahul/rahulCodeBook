...
//        BufferedReader br = new BufferedReader(new FileReader("resource\\timely_orders.txt"));
        ...
        for (int i = 0; i < nQuery ; i++) {
            ...
            if ( Integer.parseInt(qData[0]) == 2  ) {
                ...
                int nCurrentTime = Integer.parseInt(qData[1]);  // T
                int sumOfWeightOrders = getSumOfWeightOrders(nCurrentStatus, nLastMin, nCurrentTime);
    ...
    private static int getSumOfWeightOrders( int nCurrentStatus,int nLastMin,int nCurrentTime) throws Exception {

        BufferedReader bufferedReader  = new BufferedReader(new FileReader("resource\\timely_orders.txt"));
        String readLine = bufferedReader.readLine();
        int nQuery = Integer.parseInt(readLine);

        int sumWeight = 0;

        for (int i = 0; i < nQuery ; i++) {

            String qLine = bufferedReader.readLine();
            String[] qData = qLine.split(" ");



            if ( Integer.parseInt(qData[0]) == 1 && Integer.parseInt(qData[2]) <= nCurrentTime  ) {

                sumWeight = sumWeight +  Integer.parseInt(qData[1]);

            }

//            System.out.println("Processing data analytics team request.." + sumWeight );
